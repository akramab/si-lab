...
function ItemMenu ({icon, title, link}) {
    ...
    return ( 
        <a href={link} className={`w-full h-16 mt-1 flex justify-center items-center hover:cursor-pointer hover:bg-gray-300 group tooltip tooltip-right ${(location.pathname===link)? 'bg-gray-300 ':'bg-white '} `} data-tip={title}>
